	...
	unsigned int num_div)
{
	...
	// Sort according to grid index
	std::sort(acc.idx2pgi.begin(), acc.idx2pgi.end(),
		[](const ParticleGridIndex& lhs, const ParticleGridIndex& rhs)
		{ return lhs.grid_idx < rhs.grid_idx; });

	// Initialize Jagged Index Array
			...
			for (unsigned int jy = 0; jy < num_div; ++jy) {
				if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { 
					...
					for (unsigned int idx = acc.grid2idx[gidx]; idx < acc.grid2idx[gidx + 1]; ++idx) {
						...
						// Debug: Verify particle position is within grid
...
int main()
{
	...
	while (!::glfwWindowShouldClose(window)) {
		...
		if (i_step < num_step) {
			if (i_step % 20 == 0) {
				std::cout << i_step << " steps in " << num_step << " steps computed" << std::endl;
			}
