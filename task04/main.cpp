    ...
    int i_depth)
{
    ...
    if (i_depth % 2 == 0) { 
        ...
        std::sort(points.begin() + idx_point_begin, points.begin() + idx_point_end,
            [](const Eigen::Vector2f& lhs, const Eigen::Vector2f& rhs)
            { return lhs.x() < rhs.x(); });
    }
    else { 
...
void nearest_kdtree(
    ...
    int i_depth)
{
    // Early return for non-existing nodes
    ...
    // Calculate nearest postiion
    const Eigen::Vector2f pos = nodes[idx_node].pos;
    if ((pos - pos_in).norm() < (pos_near - pos_in).norm()) {
        pos_near = pos;
    }
...
int main()
{
    ...
    {                                              
        ...
        std::vector<Eigen::Vector2f> particles(20000);
