...
void wdw_spring_3d(
  ...
  const Eigen::Vector3f direction = (node2xyz[0] - node2xyz[1]) / length; // direction vector of the spring

  dw[0] = stiffness * C * direction; // gradient for the first end point
  dw[1] = -stiffness * C * direction; // gradient for the second end point
}
...
int main() {
  constexpr float learning_rate = 6.7e-3f;
